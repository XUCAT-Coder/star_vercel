<template>
  <view class="content">
    ...
    <up @linkDetail="linkDetail" ref="up" :imgurl="imgurl"></up>
  </view>
</template>
        ...
        if (i + 1 != len) {
          ...
          if (this.scrTop > tempOST1 && this.scrTop < tempOST2) {
            if (this.scrTop + 18 < tempOST1 + split) {
              ...
              this.scrTop > tempOST1 + split &&
              this.scrTop < tempOST1 + 2 * split
            ) {
              this.advance = 36;
            } else if (
              this.scrTop > tempOST1 + 2 * split &&
              this.scrTop < tempOST1 + 3 * split
            ) {
              this.advance = 54;
            } else if (
              this.scrTop > tempOST1 + 3 * split &&
              this.scrTop < tempOST1 + 4 * split
            ) {
              this.advance = 72;
            } else if (
              this.scrTop > tempOST1 + 4 * split &&
              this.scrTop < tempOST1 + 5 * split
            ) {
              this.advance = 90;
            }
            ...
            if (this.$refs.mesAll) {
              this.$refs.mesAll[
                i
              ].$el.style.transform = `rotateX(${this.advance}deg)`;
            }
          }
        } else {
          break;
